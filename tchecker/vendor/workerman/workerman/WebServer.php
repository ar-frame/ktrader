...
class WebServer extends Worker
{
     ...
     * @param string $name 
     */
    public function __construct($socket_name, $context_option = array(), $name = '')
    {
    ...
    public function onMessage($connection)
    {
        ...
        if (!isset($this->serverRoot[$_SERVER['SERVER_NAME']])) {
            foreach ($this->serverRoot as $serverConfig) {
                $entryFile = $serverConfig['entry'];
                $entryHome = dirname($entryFile);
                if (strlen($entryHome) == 1) {
                    $workerman_siteConfig = $serverConfig;
                    break;
                }
                if (strpos($_SERVER['REQUEST_URI'], $entryHome) !== false) {
                    $workerman_siteConfig = $serverConfig;
                    break;
                }
            }
        ...
        if (!is_file($workerman_file)) {
            $workerman_file = $workerman_root_dir . $workerman_siteConfig['entry'];

            if (!is_file($workerman_file)) {
                $workerman_file           = "$workerman_root_dir/index.html";
                $workerman_file_extension = 'html';
            } else {
                $entryPahtInfo = pathinfo($workerman_siteConfig['entry']);
                $workerman_file_extension = $entryPahtInfo['extension'];
            }
        }
        ...
        if (is_file($workerman_file)) {
            ...
            if ($workerman_file_extension === 'php') {
                ...
                try {
                    ...
                    $_SERVER["DOCUMENT_ROOT"] = $workerman_siteConfig['root'];
                    $_SERVER['SCRIPT_FILENAME'] = $workerman_siteConfig['root'] . $workerman_siteConfig['entry'];
                    $_SERVER["SCRIPT_NAME"]= $workerman_siteConfig['entry'];
    ...
    public static function sendFile($connection, $file_path)
    {
        ...
        $download_filename = $file_name . '.' . $extension;
