...
class Command
{
    ...
    public function run()
    {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            $pan = substr($this->dir, 0, 1);
            $commandStringPrefix = $pan . ": && ";
        }

        $commandToRun = sprintf($commandStringPrefix . 'cd %s && %s', escapeshellarg($this->dir), $this->commandString);
...
class GitRuntimeException extends \ar\core\Exception {}
