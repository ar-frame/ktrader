...
class Repository
{
    ...
    protected static $defaultOptions = array(
        'command_class'   => 'server\lib\ext\git\Command', // class used to create a command
        'git_executable'  => 'git'       // path of the executable on the server
    );
     ...
     **/
    public function getCommits($nbCommits = 10000)
    {
        $output = $this->git(sprintf('log -n %d --all --date=%s --format=format:%s', $nbCommits, $this->dateFormat, $this->logFormat));
        return $this->parseLogsIntoArray($output);
    }
     ...
     */
    public function git($commandString, $originCommand = false)
    {
        if ($originCommand === false) {
...
class InvalidGitRepositoryDirectoryException extends \ar\core\Exception
{
}
