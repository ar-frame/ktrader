...
class PaintSell:
    ...
    def gkey(self, times, timee, step, ah, aw, kh, kw):
        mongo = cfg.getMongo()
        return "nd_psell_"+mongo.get('DB')+mongo.get('DB_HOST')+"_{times}_{timee}_st{step}_{ah}_{aw}_{kh}_{kw}".format(times = times, timee = timee, step = step, ah = ah, aw = aw, kh = kh, kw = kw)
...
if __name__ == "__main__":
    ...
    timeStart = '2019-01-19 00:00:00'
    timeEnd = '2019-01-19 18:00:00'
    # timeEnd = now
    # p.getDataStr(timeStart, timeEnd, ah = 20, aw = 40, kh = 20, kw = 50)
    p.stepOver(timeStart, timeEnd, 60*30, ah = 32, aw = 50, kh = 32, kw = 76, his = True, interval = 0.01, ploop = 2) 