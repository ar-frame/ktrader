...
class Paint:
    ...
    def __init__(self, tradeVariety):
        ...
        fetch_config = Config(self.config.get("set", "API_SERVICE_ADDRESS"), self.config.get("set", "API_SERVICE_KEY"))
        self.fetch = Fetch(fetch_config)
    ...
    def drawBaseIndexCode(self, data, dataBase, ah, aw, kh, kw):
        ...
        # print(self.transNdToStr(nd_f), nd_f)

        # ader modify
        return {"nd_point_code": nd, "nd_point_str": self.transNdToStr(nd_f), "nd_arr": nd_f}
        # return nd
    ...
    def getPoint(self, timeStart, timeEnd):
        ...
        self.lastRealPointCode = dict(currentRow)
        ...
        # second_time = func.transTimedateToTime(edate) - func.transTimedateToTime(sdate)
        # f2 = '%Y%m%d%H%M%S'
        # longnowsdate = (datetime.datetime.strptime(sdate, f2) + datetime.timedelta(seconds=-second_time*4)).strftime(f2)
        # print("sec:..", second_time * 4, longnowsdate )
        ...
        if mutiny == False:
            ...
            if absMax > absMin:
                ...
                trade_opt = 'no'
                trade_msg = "code %s, balance min = max" % code
        else:
            print("had mutinied")
            trade_msg = "code %s, mutinied" % code

        return {"code": code, "trade_opt": trade_opt,"trade_msg": trade_msg, 'timedate': currentRow.timedate, "price": currentRow.price, "leftRow": leftRow, 'min': dfminRow.price, 'max': dfmaxRow.price, "nd_obj": nd_obj}

    def dump_all_nd_point_strs(self, sleep_secs) :

        for nd_point_str in self.nd_point_strs :
    ...
    def stepOrder(self, timeStart, timeEnd, timeSepM = 1):
        ...
        while True:
            ...
            lastFloatTimeSep = (datetime.datetime.strptime(timeEnd, f)+datetime.timedelta(minutes=timeSepM))
            # print(lastFloatTimeSep)

            if datetime.datetime.strptime(stimeEnd, f) <= lastFloatTimeSep:
                p.printRunTime('loop p1')

                # lasttimeLeft = (lastFloatTimeSep+datetime.timedelta(minutes=-58)).strftime(f)
                # cc = self.getPoint(lasttimeLeft, lastFloatTimeSep.strftime(f))
                # print(cc, self.lastRealPointCode)
                # exit()
                ...
                # print("st:", timeRight, datetime.datetime.strptime(timeEnd, f), timeLeft)
                ...
                if pointCode.get('trade_opt') == 'buy':
                    ...
                    print('has no point to opt', pointCode.get("code"))
        ...
        # timeStart, timeEnd, timeSepM

        # print(self.lastPointCode.get("nd_obj").get("nd_point_str"))
        try:
            cmdstr = "python3 " + sys.argv[0]
            ...
            nd_point_str = "trade_opt:%s, code:%s, trade_msg:%s\n%s" % (self.lastPointCode.get("trade_opt"), self.lastPointCode.get("trade_msg"), self.lastPointCode.get("code"), self.lastPointCode.get("nd_obj").get("nd_point_str"))
        ...
        # self.dump_all_nd_point_strs(0.05)
...
if __name__ == "__main__":
    ...
    # timeStart = '2022-06-11 06:10:00'
    timeStart = sys.argv[3]
    # timeStart = now
    # timeEnd = '2022-06-08 10:10:00'
    ...
    timeSepM = float(sys.argv[5])

    p.cha.cacheTimeArea(timeStart, timeEnd)
    # p.getDataStr(timeStart, timeEnd, ah = 22, aw = 40, kh = 22, kw = 80)
    # p.drawIndexCode(timeStart, timeEnd, ah = 22, aw = 40, kh = 22, kw = p.kw, timeSepM = 10)
    # print(p.getPoint(timeStart, timeEnd))
    p.stepOrder(timeStart, timeEnd, timeSepM = timeSepM)
