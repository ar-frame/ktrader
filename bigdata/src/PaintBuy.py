...
class PaintBuy:
    ...
    def gkey(self, times, timee, step, ah, aw, kh, kw):
        mongo = cfg.getMongo()
        return "nd_pbuy_"+mongo.get('DB')+mongo.get('DB_HOST')+"_{times}_{timee}_st{step}_{ah}_{aw}_{kh}_{kw}".format(times = times, timee = timee, step = step, ah = ah, aw = aw, kh = kh, kw = kw)
