...
# for i in range(10):
# print(gate.gateRequest(random.randint(0,99999),'depth.query',["ETH_USDT",100,"0.0001"]))
# print(gate.gateRequest(random.randint(0,99999),'depth.query',["ETH_USDT",100,"0.0001"]))
# print(gate.gateRequest(random.randint(0,99999),'depth.query',["ETH_USDT",100,"0.0001"]))
getTime = 0
while True:
    try:
        ...
        strticker = gate.gateRequest(random.randint(0,99999),'depth.query',["ETH_USDT",100,"0.0001"])
        with open('test.txt', 'a') as f:
            nowTime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')#现在
            f.write(strticker + nowTime)
        getTime = getTime + 1
    
   

        print("get time {},nowtime:{}".format(getTime, nowTime))
