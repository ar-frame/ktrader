...
class Shipan:
    ...
    def getPoint(self, timeStart, timeEnd):
        ...
        if absMax > absMin:
            ...
            if mutiny == False:
                if absMax / float(currentRow.price) < 0.0058:
                    f = '%Y-%m-%d %H:%M:%S'
            ...
            if mutiny == False:
                if absMin / float(currentRow.price) < 0.0058:
                    f = '%Y-%m-%d %H:%M:%S'
                    psLeft = datetime.datetime.strptime(timeStart, f)
                    timeLeft = (psLeft+datetime.timedelta(minutes=-60)).strftime(f)
                    return self.getPoint(timeLeft, timeEnd)
    ...
    def stepOrder(self, timeStart, timeEnd, timeSepM = 1, proxy_list = [], data_share_map = {}):
            ...
            if pointCode.get('code') > 320000:
                ...
                if len(allNotCompTrades) > 0:
                    ...
                    if lastOrder['type'] == 'buy':
                        ...
                        optres = self.trade.buy(usdT = self.initUsdtUint, price = pointCode.get('price'))
                        if optres is not None:
                ...
                if len(allNotCompTrades) > 0:
                    ...
                    if lastOrder['type'] == 'sell':
                        ...
                        if float(lastOrder['price']) < float(pointCode.get('price')):
                            ...
                            if pointPrice > 0.0038:
                                ...
                                if len(allNotCompTradesBuy) > 0:
                                    optres = self.trade.sell(usdT = spendUsdt, price = pointCode.get('price'))
                                    if optres is not None:
                        ...
                        optres = self.trade.sell(usdT = self.initUsdtUint, price = pointCode.get('price'))
                        if optres is not None:
                            # self.forcePcLast(pointCode.get('timedate'), optres.get('filledRate'), lastOrder)
    ...
    def addOrder(self, optype = 'buy', currency = 0, price = None, timedate = None, code = None, ispc = False):
        ...
        if ispc == True:
            ...
            if optype == 'buy':
                optres = self.trade.buy(currency, price)
            else:
                optres = self.trade.sell(currency, price)
    ...
    def autoStart(self, datalist, data_share_map):
        ...
        self.stepOrder(timeStart, timeStart, timeSepM = 1, proxy_list = datalist, data_share_map = data_share_map)
...
if __name__ == "__main__":
    ...
    with multiprocessing.Manager() as MG:   #重命名
        ...
        data_share_list = multiprocessing.Manager().list([])   #主进程与子进程共享这个List
        data_share_map = multiprocessing.Manager().dict()
        
        p = multiprocessing.Process(target=sp.autoStart,args=(data_share_list, data_share_map))
