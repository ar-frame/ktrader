...
class Paint:
    ...
    def __init__(self, initStockName):
        ...
        self.sim = Simulate(initStockName)
        self.cha = Characteristic(initStockName)
        self.ts = TradeSave()
        self.threads = []
    ...
    def getPoint(self, timeStart, timeEnd):
        ...
        # print("absMax %f" % absMax)
        # print("absMin %f" % absMin)
       
        mutiny = False
        if absMax > absMin:
            leftRow = dfmaxRow

            if absMin / float(currentRow.price) > 0.01:
                if dfminRow.timedate > dfmaxRow.timedate:
                    leftRow = dfminRow
                    mutiny = True

            if mutiny == False:
                if absMax / float(currentRow.price) < 0.0018:
                    f = '%Y-%m-%d %H:%M:%S'
            ...
            if absMax / float(currentRow.price) > 0.01:
                if dfmaxRow.timedate > dfminRow.timedate:
                    leftRow = dfmaxRow
                    mutiny = True

            if mutiny == False:
                if absMin / float(currentRow.price) < 0.0018:
                    f = '%Y-%m-%d %H:%M:%S'
                    psLeft = datetime.datetime.strptime(timeStart, f)
                    timeLeft = (psLeft+datetime.timedelta(minutes=-60)).strftime(f)
                    return self.getPoint(timeLeft, timeEnd)
    ...
    def checkTimeIsTradeTime(self, checktime):
        f = '%Y-%m-%d %H:%M:%S'
        checktimePtime = datetime.datetime.strptime(checktime, f)
      
        # 范围时间
        d_am_time = datetime.datetime.strptime(func.transF1ToDay(checktime)+'09:25:00', '%Y%m%d%H:%M:%S')
        d_am_time_end = datetime.datetime.strptime(func.transF1ToDay(checktime)+'11:30:00', '%Y%m%d%H:%M:%S')


         # 范围时间
        d_pm_time = datetime.datetime.strptime(func.transF1ToDay(checktime)+'13:00:00', '%Y%m%d%H:%M:%S')
        d_pm_time_end = datetime.datetime.strptime(func.transF1ToDay(checktime)+'15:00:00', '%Y%m%d%H:%M:%S')

        if checktimePtime.isoweekday() == 6 or checktimePtime.isoweekday() == 7:
            return {"t": False, 'r': "wk"}
        else:
            if (checktimePtime < d_am_time) :
                return {"t": False, 'r': "am"}
            elif checktimePtime >= d_am_time and checktimePtime < d_am_time_end:
                return {"t": True, 'r': "am"}
            elif checktimePtime >= d_am_time_end and checktimePtime < d_pm_time:
                return {"t": False, 'r': "noon"}
            elif checktimePtime >= d_pm_time and checktimePtime < d_pm_time_end:
                return {"t": True, 'r': "pm"}
            else:
                return {"t": False, 'r': "pm"}


    def getLastWorkDay(self, dayf1):
        f = '%Y-%m-%d %H:%M:%S'
        pf = datetime.datetime.strptime(dayf1, f)
        # print(now)
        # print(now.isoweekday())

        if pf.isoweekday() == 1:
            dayStep = 3
        elif pf.isoweekday() == 7:
            dayStep = 2
        else:
            dayStep = 1

        lastWorkDay = pf - datetime.timedelta(days = dayStep)
        
        lastWorkDay = lastWorkDay.strftime(f)
        return lastWorkDay[0:10]

    def getNextWorkDay(self, dayf1):
        f = '%Y-%m-%d %H:%M:%S'
        pf = datetime.datetime.strptime(dayf1, f)
        # print(now)
        # print(now.isoweekday())
    ...
    # 当天下次开盘日期
    def getNextDateOpenDaytime(self, datef1):
        ts = self.checkTimeIsTradeTime(datef1)
        if (ts.get('t') == False):
            if ts.get('r') == 'am':
                return datef1[0:10] + " 09:25:00"
            elif ts.get('r') == 'noon':
                return datef1[0:10] + " 13:00:00"

            elif ts.get('r') == 'wk':
                return self.getNextWorkDay(datef1) + " 09:25:00"
    ...
    # 当天上次最近开盘日期
    def getLastDateOpenDaytime(self, datef1):
        ts = self.checkTimeIsTradeTime(datef1)
        if (ts.get('t') == False):
            if ts.get('r') == 'am':
                return self.getLastWorkDay(datef1) + " 13:00:00"
            elif ts.get('r') == 'noon':
                return datef1[0:10] + " 09:25:00"
    ...
    # 当天下次收盘日期
    def getDateCloseDaytime(self, datef1):
        ts = self.checkTimeIsTradeTime(datef1)
        if (ts.get('t') == False):
            if ts.get('r') == 'am':
                return datef1[0:10] + " 11:30:00"
            elif ts.get('r') == 'noon':
                return datef1[0:10] + " 15:00:00"
            elif ts.get('r') == 'wk':
               
                return self.getNextWorkDay(datef1) + " 11:00:00"
            else:
                return self.getNextWorkDay(datef1) + " 11:00:00"
        else:
            return self.getNextWorkDay(datef1) + " 15:00:00"
            

    # 当天上次最近收盘日期
    def getLastDateCloseDaytime(self, datef1):
        ts = self.checkTimeIsTradeTime(datef1)
        if (ts.get('t') == False):
            if ts.get('r') == 'am':
                return self.getLastWorkDay(datef1) + " 15:00:00"
            elif ts.get('r') == 'noon':
                return datef1[0:10] + " 11:30:00"
    ...
    def getOpenTimeMinutes(self, datef1):
        ts = self.checkTimeIsTradeTime(datef1)
        ms = 0
        if (ts.get('t') == True):
            if ts.get('r') == 'am':
                ms = (func.transDateF1ToTime(datef1) - func.transDateF1ToTime(datef1[0:10] + " 09:25:00")) / 60
               
            elif ts.get('r') == 'pm':
                ms = (func.transDateF1ToTime(datef1) - func.transDateF1ToTime(datef1[0:10] + " 13:00:00")) / 60
    ...
    def stepOrder(self, timeStart, timeEnd, timeSepM = 1):
        ...
        ts = self.checkTimeIsTradeTime(timeStart)
        if ts.get('t') == True:
            print("交易时间")
        else:
            print("非交易时间")
            timeStart = self.getNextDateOpenDaytime(timeStart)
        ...
        while True:
            ...
            if datetime.datetime.strptime(stimeEnd, f) <= datetime.datetime.strptime(timeEnd, f):
                ...
                ts = self.checkTimeIsTradeTime(stimeEnd)

                if ts.get('t') == True:
                    hasopenM = self.getOpenTimeMinutes(stimeEnd)
                    if hasopenM >= 158:
                        psLeft = datetime.datetime.strptime(timeRight, f)
                        timeLeft = (psLeft+datetime.timedelta(minutes=-158)).strftime(f)
                    else:
                        lastCloseTime = self.getLastDateCloseDaytime(stimeEnd)
                        lssep = 158 - hasopenM
                        psLeft = datetime.datetime.strptime(lastCloseTime, f)
                        timeLeft = (psLeft+datetime.timedelta(minutes=-lssep)).strftime(f)
                    ...
                    # outS = func.getTimeSepMinutes(stimeEnd) - func.getTimeSepMinutes(lastCloseTime)
                    pass
                ...
                ts2 = self.checkTimeIsTradeTime(stimeEnd)

                if ts2.get('t') == False:
                    lastCloseTime = self.getLastDateCloseDaytime(stimeEnd)

                    print(stimeEnd, lastCloseTime)
                    outS = func.getTimeSepMinutes(stimeEnd, lastCloseTime)
                    nextOpenTime = self.getNextDateOpenDaytime(stimeEnd)

                    ps = datetime.datetime.strptime(nextOpenTime, f)
                    stimeEnd = (ps+datetime.timedelta(minutes=outS)).strftime(f)
    ...
    def getTradeStr(self, timedate, price):
        ...
        if len(df) > 0:
            ...
            # print("已实现盈利 %.2f" % c_profit)
            # print("未实现盈利 %.2f" % profitFeature)
            

            showStr = showStr + ("总单数 %d , 多单 %d, 空单 %d" % (len(df), len(df_buy), len(df_sell))) + "\n"

            # print("完成单数 %d" % len(df_complete))
            # showStr = showStr + ("成功单数 %d" % len(df_succ)) + "\n"
            # showStr = showStr + ("失败单数 %d" % len(df_fail)) + "\n"

            # print("挂单数 %d" % len(df_maker))
            # if len(df) > 0:
            #     sratio = len(df_succ) / len(df) * 100
            #     showStr = showStr + ("成功率 %.2f%%" % sratio) + "\n"
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) < 3) :
        print('usdt or stockname not set')
        exit()
    ...
    initStockName = sys.argv[2]

    p = Paint(initStockName)


    # 下次的开盘时间
    sd = p.getDateCloseDaytime('2019-04-20 16:24:12')
    print(sd)
    sd2 = p.getLastDateCloseDaytime('2019-04-20 16:24:12')
    print(sd2)
    # exit()
    ...
    timeStart = '2018-01-05 00:00:00'
    timeEnd = '2018-05-20 23:59:00'
    ...
    p.stepOrder(timeStart, timeEnd, timeSepM = 40)